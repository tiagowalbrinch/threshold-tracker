...
public interface IUserTaskStatService
{
    ...
    Task<UserTaskStatResponse> SetThresholdAsync(string userId, string aimlabTaskId, int value, CancellationToken ct = default);
    Task<IReadOnlyList<LeaderboardEntryResponse>> GetLeaderboardAsync(string aimlabTaskId, CancellationToken ct = default);
